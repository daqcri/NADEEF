...
public class MyRule4 extends PairTupleRule {
    ...
    public void initialize(String id, List<String> tableNames) {
        ...
        leftHandSide.add(new Column("TB_HOSPITAL_30K.zipcode"));

        rightHandSide.add(new Column("TB_HOSPITAL_30K.city"));
     ...
     */
    @Override
    public void iterator(Collection<Table> tables, IteratorResultHandler iteratorBlockingQueue) {
        Table table = tables.iterator().next();
                ...
                if (findViolation) {
                    for (int i = pos1; i < pos2; i ++) {
                        for (int j = pos2; j < table.size(); j++) {
                            ...
                            iteratorBlockingQueue.handle(pair);
                        }
                    }
                    break;
                }
